function [X_k,Y_k,Z_k] = ECEF_from_RINEX(t0e, GM, Omega_e, satellitt_data)
...
% Keplers likning for eksentrisk anomali
Ek = Mk;
for i = 1:50
    Ek = Mk + eccentricity * sin(Ek);
end

% Finn sann anomali, atan2 for a finne rett kvadrant:
vk_cos = (cos(Ek)-eccentricity)/(1-eccentricity*cos(Ek)); 
vk_sin = (sqrt(1-eccentricity^2)*sin(Ek))/(1-eccentricity*cos(Ek)); 
